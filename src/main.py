...
def generate_chatgpt_analysis(chat_history, profile):
    """Gera análise usando ChatGPT com base no histórico de chat."""
    if not client:
    ...
    system_message = {
        "role": "system",
        "content": f"You are Ralph, an expert AI business analyst for real estate professionals. You are analyzing a conversation with a {context}. Provide actionable, specific advice based on the conversation data."
    }

    messages = [system_message]

    # Adiciona o histórico da conversa formatado
    for msg in chat_history:
        sender = msg.get("sender")
        content = msg.get("content", "").strip()
        # Remove HTML simples e pula mensagens vazias ou iniciais
        clean_content = re.sub("<.*?>", "", content).strip()
        if not clean_content or "To start, please tell me" in clean_content:
            continue
    ...
    # Adiciona o prompt final solicitando a análise
    analysis_prompt = f"""Based *only* on the conversation history provided above, provide a comprehensive business analysis for the {context}. Your analysis must include:
...
Analysis:"""
    ...
    try:
        print("\n--- Enviando requisição para OpenAI API com histórico de chat ---")
        # print(f"DEBUG: Mensagens enviadas: {json.dumps(messages, indent=2)}") # Descomentar para depuração
        ...
        if not ai_analysis_text:
            return "Analysis could not be generated. Empty response received."
...
def analyze_data():
    ...
    try:
        ...
        if not client:
            ...
            ai_analysis_text = generate_chatgpt_analysis(chat_history, profile)
